  ...
  class Shell {
    ...
    async exec(cmdLine) {
      ...
      switch (action) {
        ...
        case "cat": {
          const target = rest[0];
          return this._protectedWrapper(target, isSudo, this._cat);
        }
        case "vim":
        ...
        default:
          return `shell: command not found: ${cmdLine}`;
      }
    }
    ...
    async _mkdir(dir, sudoFlag, isSudo) {
      ...
      const payload = {
        [key]: { [this._keyFromName("DONOTDELETE")]: "NODELETE" }
      };
